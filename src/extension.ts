...
import { SidelineProvider } from './sidelineProvider';
import { LiveGameTracker } from './liveGameTracker';
import { TrackedGamesProvider } from './trackedGamesProvider';

export function activate(context: vscode.ExtensionContext) {
    console.log('Sideline extension is now active!');

    // Create the live game tracker
    const gameTracker = new LiveGameTracker();
    
    // Create the provider
    const provider = new SidelineProvider(context.extensionUri, gameTracker);
